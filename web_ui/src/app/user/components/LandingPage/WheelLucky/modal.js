  ...
  const validateReqire = (field, fieldsT, errorsT, fiels = []) => {
    fiels.forEach((item) => {
      if (field === item || !field) {
        if (
          !fieldsT[item] ||
          (Array.isArray(fieldsT[item]) && !fieldsT[item][0])
        )
    ...
    <>
      <Modal show={true} size="lg">
        <Modal.Header>
          ...
          <Modal.Title>
            <div>Nhập thông tin quay thưởng</div>
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
        ...
        <Modal.Footer className="mb-3">
          <button
            type="button"
            className="btn btn-primary btn-warning btn-lg"
            onClick={confirm}
          >
            Xác nhận
          </button>
