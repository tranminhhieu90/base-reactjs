...
export const ORDER_STATUS = ['Đơn hàng mới', 'Đã duyệt', 'Không được thông qua', 'Đã đặt hàng', 'Tại kho Mỹ', 'Tại kho Việt Nam', 'Đã giao cho khách', 'Đơn hàng bị hủy'];
...
export const formatDateToInput = (date = new Date()) => {
    ...
    const day = d.getDate() < 10 ? '0' + d.getDate() : d.getDate();
    const mont = (d.getMonth() + 1) < 10 ? '0' + (d.getMonth() + 1) : d.getMonth() + 1;
    return `${d.getFullYear()}-${mont}-${day}`;
}
export const formatInputToDate = (date) => {
...
export const SCOPE_ACCESSES = [
    { value: 'EMPLOYEE', text: 'Nhân viên' },
    { value: 'WRITER', text: 'Viết blog' },
